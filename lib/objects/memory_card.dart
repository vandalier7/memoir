...
class _MemoryCardState extends State<MemoryCard> with SingleTickerProviderStateMixin {
  ...
  Future<void> _sendComment() async {
    ...
    if (text.isEmpty || _isPostingComment) return;  // ADD _isPostingComment check
    ...
    setState(() => _isPostingComment = true);  // ADD THIS LINE
    ...
    try {
      ...
      setState(() {
        ...
        _replyingToCommentId = null;
        _replyingToUserName = null;
        _isPostingComment = false;  // ADD THIS LINE
      });
      ...
      setState(() => _isPostingComment = false);  // ADD THIS LINE
    }
  }
...
Widget _buildPrivacyOption(int level, String label, IconData icon) {
  final isSelected = _currentPrivacyLevel == level;
  ...
  return InkWell(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        color: isSelected ? memoirTheme.tertiary.withOpacity(0.2) : null,
      ...
      child: Row(
        children: [
          Icon(
            icon,
            size: 20,
            color: isSelected ? memoirTheme.tertiary : Colors.grey.shade900,
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Text(
              ...
              style: TextStyle(
                fontSize: 14,
                color: isSelected ? memoirTheme.tertiary : Colors.grey.shade800,
                fontWeight: isSelected ? FontWeight.w600 : FontWeight.w500,
              ),
            ),
          ),
          if (isSelected)
            Icon(Icons.check, size: 20, color: memoirTheme.tertiary),
        ],
      ),
    ),
  );
                        ...
                        children: [
                          if (currentUser != null)
                            StreamBuilder<Map<String, dynamic>>(
                              stream: databaseService.getUserStream(currentUser.uid),
                              builder: (context, snapshot) {
                                String? avatarUrl;
                                String username = '';
